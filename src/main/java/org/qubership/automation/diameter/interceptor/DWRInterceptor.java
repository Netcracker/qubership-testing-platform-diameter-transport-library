...
public class DWRInterceptor extends Interceptor {
    private static final Logger LOGGER = LoggerFactory.getLogger(DWRInterceptor.class);
    private final String dwa;
    private final Encoder encoder;

    public DWRInterceptor(String dwa, Encoder encoder) {
     ...
     */
    public DWRInterceptor(boolean deleteOnReceive, String dwa, Encoder encoder) {
        super(deleteOnReceive, InterceptorTypes.DWR);
        this.dwa = dwa;
    ...
    public boolean _onReceive(String message, ExtraChannel channel) {
        if (StringUtils.startsWithIgnoreCase(message, "<DWR>")) {
            LOGGER.debug("DWR Received:\n{}", message);
            try {
                ...
                LOGGER.debug("DWA Sent:\n{}", dwa);
            } catch (Exception e) {
                LOGGER.error("Unable to encode DWA message", e);
            }
        }
        return false;
    }
